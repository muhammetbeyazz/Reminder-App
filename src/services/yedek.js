...
// Sürekli çalan alarm sesi için fonksiyon
const playAlarmSoundLooped = (soundFileName) => {
  const alarmSound = new Sound(soundFileName, Sound.MAIN_BUNDLE, (error) => {
    ...
    // Alarm sesini sürekli döngüye sok
  ...
  Object.keys(tasks).forEach((taskId) => {
    ...
    if (reminderTime >= now - oneMinute && reminderTime <= now && task.status === 'aktif') {
      ...
      if (reminderTime - now <= oneMinute) {
        const selectedAlarmSound = task.reminderSound || 'default_alarm.mp3';
        playAlarmSoundLooped(selectedAlarmSound);  // Sürekli çalan alarm sesini burada başlat
      }
    }
  });
};
